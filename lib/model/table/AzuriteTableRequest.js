...
class AzuriteTableRequest {
  constructor({ req = undefined, payload = undefined }) {
    ...
    if (req.query.NextPartitionKey !== undefined) {
      this.nextPartitionKey = decodeURIComponent(req.query.NextPartitionKey);
    }

    if (req.query.NextRowKey !== undefined) {
      this.nextRowKey = decodeURIComponent(req.query.NextRowKey);
    }
  }
  ...
  _initHttpProps(httpHeaders) {
    ...
    if (httpHeaders[N.TABLE_CONTINUATION_NEXT_PARTITION_KEY] !== undefined &&
      httpHeaders[N.TABLE_CONTINUATION_NEXT_ROW_KEY] !== undefined) {
      this.httpProps[N.TABLE_CONTINUATION_NEXT_PARTITION_KEY] = decodeURIComponent(httpHeaders[N.TABLE_CONTINUATION_NEXT_PARTITION_KEY]);
      this.httpProps[N.TABLE_CONTINUATION_NEXT_ROW_KEY] = decodeURIComponent(httpHeaders[N.TABLE_CONTINUATION_NEXT_ROW_KEY]);
    }
  }
