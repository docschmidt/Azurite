...
class TableStorageManager {
  ...
  queryEntities(request) {
    ...
    // handle continuation
    if (request.nextPartitionKey)
    {
      findExpr["partitionKey"] = { "$gte": request.nextPartitionKey };
    }
    if (request.nextRowKey)
    {
      findExpr["rowKey"] = { "$gte": request.nextRowKey };
    }
    ...
    for (const item of find) {
      // return at most top items
      if ( numItems < request.top )
      {
    ...
    if ( find.length > numItems )
    {
      response.continuationToken = {
        nextPartitionKey: find[numItems].partitionKey,
        nextRowKey: find[numItems].rowKey
      };
    }

    return BbPromise.resolve(new AzuriteTableResponse(response));
  }
